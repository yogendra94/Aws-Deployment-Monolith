...
const Cart = () => {
        ...
        const cartItemsWithImages = await Promise.all(
          updatedCartItems.map(async (item) => {
            try {
              ...
              const imageFile = await converUrlToFile(imgResponse.data, item.imageName);
  ...
  const converUrlToFile = async (blobData, fileName) => {
    const file = new File([blobData], fileName || "image", { type: blobData.type });
    return file;
  };
  ...
  const handleCheckout = async () => {
    try {
      for (const item of cartItems) {
        ...
        // ✅ FIXED: Blob wrapping for checkout update call too
        cartProduct.append(
          "product",
